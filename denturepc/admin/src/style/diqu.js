...
enterpriseinformation_province_add();


//添加省份
function enterpriseinformation_province_add() {
    var str = "";
    for (var i = 0; i < provinceName.length;i++){
        str += '<option value="' + provinceName[i] + '">' + provinceName[i] + '</option>'
    }
    $("#enterpriseinformation_province").html(str);
    enterpriseinformation_homecity_add(city1)
    form.render('select');

}

//渲染市区
function enterpriseinformation_homecity_add(p) {
    var list = p;
    var str = "";
    ...
    $("#enterpriseinformation_homecity").html(str);
    form.render('select');
}
form.on('select(enterpriseinformation_province)', function (data) {
    var index
    for (var i = 0; i < provinceName.length; i++) {
        if (provinceName[i] == data.value) {
             index = eval("city" + Number(i+1))
        }
    }
    enterpriseinformation_homecity_add(index); //得到被选中的值)
});